...
class CalendarWidget extends FullCalendarWidget
{
    ...
    public function onEventDrop(array $event, array $oldEvent, array $relatedEvents, array $delta, ?array $oldResource, ?array $newResource): bool
    {
        ...
        if (Carbon::parse($event['start'])->format('Y-m-d') < $this->record->start->format('Y-m-d') && $this->record->start->format('Y-m-d') < '2024-08-12') {
            Notification::make()
            ...
            Forms\Components\Grid::make()
                ...
                ->schema([
                        ...
                        ->options(function () {
                            if (!Gate::allows('special-events')) {
                                return collect(Events::cases())
                                    ->filter(fn($case) => $case !== Events::SUS && $case !== Events::HOL && $case !== Events::FLAG)
                    ...
                    \Filament\Forms\Components\Select::make('hris_number')
                        ->label('Employee Name')
                        ->multiple()
                        ->getSearchResultsUsing(fn(string $search): array => Employee::departmentCovered()->searchEmployee($search)->limit(50)->get()->pluck('full_name', 'hris_number')->toArray())
                ...
                ->schema([
                    \Filament\Forms\Components\Select::make('tag')
                        ...
                        ->options(function () {
                            if (!Gate::allows('special-events')) {
                                return collect(Events::cases())
                                    ->filter(fn($case) => $case !== Events::SUS && $case !== Events::HOL && Events::FLAG)
                                    ->mapWithKeys(fn($case) => [$case->value => $case->getLabel()])
                                    ->toArray();
                            }
                    ...
                    \Filament\Forms\Components\Select::make('hris_number')
                        ...
                        ->getSearchResultsUsing(fn(string $search): array => Employee::isDapcc()->departmentCovered()->searchEmployee($search)->limit(50)->get()->pluck('full_name', 'hris_number')->toArray())
